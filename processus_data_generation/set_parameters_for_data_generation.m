...
N = [255 255];                                       % Image dimension (px)
field_of_view = [-0.01905 0.01905 0.005 0.0431];     % Field of view (m)
use_eir = false;                                      % Electrical impulse response
use_sir = false;                                     % Spatial impulse response
use_single_speed_of_sound = true;                    % Indicate whether the probe couplant should be defined by a specific SoS
filt_min = 4.8e6;                                    % Lower bound of the bandpass filter (Hz)
filt_max = 10.8e6;                                   % Higher bound of the bandpass filter (Hz)
num_cropped_samples_at_sinogram_start = 0;           % Number of samples that are cropped from the beginning of the sinogram (in signal samples)
...
lambda_shearlet = 0;                                 % Strength of Shearlet L1 regularization, only used if regularization = 'L1_SHEARLET' 
